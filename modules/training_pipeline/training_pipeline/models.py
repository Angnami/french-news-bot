...
def download_from_model_registry(model_id:str, cache_dir:Optional[Path]=None):
    ...
    logger.info(msg=f"Le modèle {model_id} est téléchargé depuis le registre dans {model_dir}")
...
def prompt(
    ...
    """
    inputs = tokenizer(input_text=input_text, return_tensors='pt', return_token_type_ids=False).to(device)

...
class BestModelToModelRegistryCallback(TrainerCallback):
    ...
    def on_train_end(
        ...
        **kwargs,
        ...
        if has_best_model_checkpoint:
            ...
            logger.info(msg=f"Enregistrement du melleur modèle à partir de {best_model_checkpoint} dans le registre de modèle.")
        else:
            logger.warning(msg="Pas de meileure version trouvée. L'enregistrement dans le regsitre n'est pas effectué.")
    ...
    def to_model_registry(self, checkpoint_dir:Path):
        ...
        logger.info(msg=f"Début de l'enregistrement du checkpoint du modèle @ {self.model_name}")
...
class TrainingAPI:
    ...
    def load_data(self)->Tuple[Dataset, Dataset]:
        ...
        """
        logger.info(msg=f"Chargement des Datasets de QR à partir {self._root_dataset_dir=}")

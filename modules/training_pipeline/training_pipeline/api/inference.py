...
class InferenceAPI:
    ...
    def load_data(self) -> Dataset:
        ...
        """
        logger.info(
            msg=f"Récupération du dataset des QA à partir de {self._root_dataset_dir=}"
        )
        if self._debug:
            max_samples = 3
        else:
    ...
    def load_model(self) -> Tuple[AutoModelForCausalLM, AutoTokenizer, PeftConfig]:
        ...
        """
        logger.info(
            msg=f"Chargement du modèle {self._model_id=} et de la {self._peft_config=}"
        )
    ...
    def infer_all(self, output_file: Optional[Path] = None) -> None:
        ...
        for sample in tqdm(self._dataset):
            ...
            if should_save_output:
                promt_and_answers.append({"prompt": sample["prompt"], "answer": answer})
        if should_save_output:
